...
do
    ...
    echo "LOG: Generating pdf for $name..."
    pdflatex -jobname="$name" Resume_LaTeX.tex
    # Try to concatinate academic_transcript.pdf onto cover letter/resume
    echo "LOG: Concatenating academic_transcript.pdf"
    pdftk "$name.pdf" academic_transcript.pdf cat output "$name2.pdf"
    ...
    if [ $? -ne 0 ]
    then
	echo "LOG: Failed to find academix transcipt, moving resume as is to Resumes folder..."
        mv "$name.pdf" ./Resumes/"$name Cover Letter and Resume.pdf"
    else
	echo "LOG: Academic transcript added, moving resume to Resumes folder..."
        mv "$name2.pdf" ./Resumes/"$name Cover Letter and Resume.pdf"
        rm "$name.pdf"
    fi
done
